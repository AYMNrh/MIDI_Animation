...
class Square:
    ...
    def compute_glowy_surface(self, rect, val):
        if Config.rainbow_glow:
            # Calculate rainbow color based on time
            hue = (pygame.time.get_ticks() / Config.rainbow_glow_speed) % 360
            rainbow_color = pygame.Color(0, 0, 0)
            rainbow_color.hsva = (hue, 100, 100, 100)
            glow_color = rainbow_color
        else:
            glow_color = Config.glow_color  # Use the configured static glow color

        glowy_borders = make_glowy2((rect.size[0] + 40, rect.size[1] + 40), glow_color, val)
